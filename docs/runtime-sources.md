...
```bash
cargo run -p create-axonix -- my-app --yes --runtime-source path
```
...
- `create-axonix` creates the app
- `axonix-runtime` is the runtime package the app depends on
- `axonix-framework` is where the framework direction is still being shaped
