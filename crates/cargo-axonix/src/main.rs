...
#[derive(Debug, Parser)]
#[command(name = "cargo-axonix")]
#[command(bin_name = "cargo-axonix")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn add_module(module: ModuleKind) -> Result<()> {
    ...
    let axonix_toml = root.join("Axonix.toml");
    ...
    match module {
        ModuleKind::Docs => {
            ...
            enable_module(&axonix_toml, "docs")?;
...
fn enable_module(axonix_toml: &PathBuf, module_name: &str) -> Result<()> {
    let source = fs::read_to_string(axonix_toml)
        .with_context(|| format!("failed to read '{}'", axonix_toml.display()))?;
    let mut value = source
        .parse::<toml::Value>()
        .with_context(|| format!("failed to parse '{}'", axonix_toml.display()))?;

    let root_table = value
        .as_table_mut()
        .ok_or_else(|| anyhow::anyhow!("Axonix.toml root must be a TOML table"))?;
    ...
    let rendered = toml::to_string_pretty(&value).context("failed to render Axonix.toml")?;
    fs::write(axonix_toml, rendered)
        .with_context(|| format!("failed to write '{}'", axonix_toml.display()))?;
    Ok(())
}
